...
# Ask holo or bin
def ask_question():
    # This inner function will be our custom dialog
    class ChoiceDialog(simpledialog.Dialog):
        
        def body(self, master):
            self.result = "cancel"
            tk.Label(master, text="What do you want to feed to the crocodile? Holograms or bin-files?").grid(row=0, columnspan=3, pady=10)
        ...
        def buttonbox(self):
            ...
            btn1 = tk.Button(box, text="Holograms", width=30, command=lambda: self.set_choice("H"))
            btn1.pack(side=tk.LEFT, padx=5)

            btn2 = tk.Button(box, text="Bin-files", width=30, command=lambda: self.set_choice("B"))
            btn2.pack(side=tk.LEFT, padx=5)

            btn3 = tk.Button(box, text="Cancel", width=10, command=lambda: self.set_choice("C"))
        ...
        def set_choice(self, value):
            if value == "H":
                self.result = "holograms"
            elif value == "B":
                self.result = "bin-files"
            else:
                self.result = "cancel"
            self.destroy()

    groot = tk.Tk()
    groot.withdraw()  # Hide main window

    dialog = ChoiceDialog(groot, title="Make a Choice")
    groot.destroy()  # Properly close the hidden main window
    return dialog.result

FileType = ask_question()

print(FileType)

if not FileType == "cancel": 
    ...
    def save_in_folder():
        global in_folder
        in_folder=tk.filedialog.askdirectory(parent=root, title=f"Chose a folder with {FileType}")
        ...
        if tsstring != 'No timestamps file selected.\n':
            ...
            if NAV.isdigit()==True and NAV!='0':
                if int(NAV)>sequence_length:
                    ...
                    displaytext_NAV=f'\nNumber of {FileType} to average cannot be greater than sequence length.'
                    displaytext_new_N ='\nTarget sequence length = 1'
                    displaytext_RFR=''
                else:
                    if RFR.isdigit()==True and RFR!='0':
                        if int(NAV)<=int(RFR):
                            displaytext_NAV=f'\nAverage over {NAV} {FileType}'
                        else:
                            displaytext_NAV=f'\nAverage over {NAV} {FileType}'
            ...
            if os.path.isfile(time_file)==False:
                ...
                if RFR.isdigit()==False or RFR=='0':
                    ...
                    if NAV.isdigit()==False or NAV=='0':
                        tk.messagebox.showinfo('Error', f'The entry for {FileType} to average must be a positive integer!')
                        NAV_entry.delete(0,tk.END)
                        ...
                        if outputoption == "overwrite":
                            result = tk.messagebox.askquestion(f'Overwrite {FileType}?', f'Are you sure that you want to overwrite the intial {FileType}?')
                            if result == 'yes':
                                #call the croco proceedure with input overwrite = True
                                PROD_croco(in_folder,in_folder,FileType,True,time_file,int(RFR),int(NAV),root)
                                        ...
                                        if new_entry.get() == in_folder:
                                            ...
                                            if len(os.listdir(new_entry.get())) != 0:
                                                ...
                                                if result3 == "yes":
                                                    ...
                                                    PROD_croco(in_folder,new_entry.get(),FileType,False,time_file,int(RFR),int(NAV),root)
                                            else:
    ...
    in_button = tk.Button(FP_frame, text=f"Chose a folder with {FileType}", width=32, height=1, command=lambda: save_in_folder())
    ...
    NAV_entry.insert(0,"1")
    NAVV_label= tk.Label(FP_frame, text= f"{FileType}")
    
    RFR_entry.bind("<KeyRelease>", update_display)
    NAV_entry.bind("<KeyRelease>", update_display)
    
    Vover=tk.BooleanVar()
    over = tk.Checkbutton(FP_frame, text=f"Overwrite intial {FileType}", variable=Vover, command=lambda: check_option('over'))
    Vnew=tk.BooleanVar()
